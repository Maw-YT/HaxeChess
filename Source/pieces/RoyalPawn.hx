...
/**
 * RoyalPawn: A pawn with royal status (game ends if captured)
 * Moves like a normal pawn (forward movement, diagonal capture, en passant)
 * But is marked as royal like a King
 */
class RoyalPawn extends BasePiece {
    public function new(color:String) {
        super("royalpawn", color);
        setRoyal(true);
    }
            ...
            if (onEnPassantRank) {
                ...
                if (boardManager != null) {
                    ...
                    if (lastMoveStart != null && lastMoveEnd != null && lastMovedPiece != "") {
                        ...
                        // Check if last moved piece can be passanted
